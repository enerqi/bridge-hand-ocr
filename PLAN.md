...
- **2026-07-16 (session 11).** Multi-render atlases + the resolution lever proven,
  ...
  + an HTML-render print screenshot).
  - **Resolution is the print lever (measured, then proven).** Median rank-glyph
    height vs valid: 7px→0%, 12-13px→~30-40% (all *regular* print grids), 40-51px
    →exact. The user's high-res exports land at ~20-24px. With a same-render
    `print-zoomed` atlas (built from print-5x6-format-zoomed board 1) they jump:
    **print-4x5-zoomed-format-high 0→8/8 (perfect)**, print-3x4-format-zoomed
    6/7. `read_rows` auto-selects print vs print-zoomed by measured glyph height
    (`_median_glyph_height` ≥ 16px → zoomed), so no new hint plumbing.
  - **Combined-render atlases beat per-render routing.** A grid tile renders
    smaller than a standalone board; rather than route to a separate atlas, the
    `bridgewebs` atlas is now built from *both* bridgewebs-4-2 and 3x3-multi board
    1 (`build_atlas` takes multiple images now). Nearest-exemplar matches whichever
    render a query came from → **part2 8→9/9**, and the HTML-render print
    screenshot (green compass, white bg, same deals as the zoomed set) reads
    **5/6** on the same atlas with zero routing. This replaced the session-draft
    `bridgewebs-grid` hint (which mis-served the HTML render 4/6).
  - Floors raised (part2 9/9, multi-table 6/6); +3 tests (87): 2 high-res print
    floors, 1 HTML-print floor. ruff / ty clean. New `print-zoomed` atlas dir.
    Remaining print gap is the ~12px regular grids (segmentation-bound) and the
    5x6-zoomed edge/void tiles (4/11); high-res is the answer where available.
...
## Where we are (verified 2026-07-16, sessions 2-11 applied)

**87 tests pass.** Live fixture sweep — reproduce with **`just sweep`** (all
fixtures) or `just sweep fixtures/<glob>`. Column key: **valid/total** =
legal-deal boards / boards found; **exact** = additionally pinned against a
ground-truth PBN sidecar (`fixtures/expected/`, 11 fixtures) by the regression
harness. "valid but unpinned" = a legal deal read that isn't sidecar-verified.

| Fixture | valid/total | Notes |
|---|---|---|
| `bridgewebs-4.png`, `bridgewebs-4-2.png` | 1/1 | four hands **exact** (compass anchor) |
| `bridgewebs-4-3x3-multi.png` | 6/9 | grid tiling; render-mismatch tail |
| `bridgewebs-4-3x3-multi-part2.png` | **9/9** | multi-render atlas win (session 11) |
| `bridgewebs-4-multi-table.png` | 6/6 | all six board panels legal |
| `realbridge-4-results.png` | 1/1 | **exact** (suit-quadruple anchor + own atlas) |
| `realbridge-replay-1.png`, `-2-nonvul.png` | 1/1 | **exact** + full metadata tags (Board/Dealer/Vul/Contract/Declarer/Result) |
| `print-4x5-zoomed-format-high.png` | **8/8** | high-res print → perfect (resolution lever, session 11) |
| `print-3x4-format-zoomed.png`, `print-3xn-format-html.png` | 5/6 | zoomed / HTML-render prints |
| `print-3x4-format.png` | 3/12 | board 1 **exact**; rest = ~12px segmentation tail |
| `print-4x5-zoomed-format.png`, `print-5x6-format-zoomed.png` | 3/12, 4/10 | mixed zoom |
| `print-5x6-format.png`, `print-4x5-format.png` | 7/18, 0/18 | regular low-res — segmentation-bound |
| CARDS `bridge-base-2-hand-large`, `-small` | 1/1 | N/S **exact**; W/E card-backs → `-` |
| CARDS `bridge-base-4-hand-large`, `-small` | 1/1 | **4/4 exact** (strips + grids) |
| CARDS `bridge-base-4-hand-very-small` | 1/1 | legal read, **unpinned** (was soft-fail pre-s11) |
| CARDS `intobridge-4-hand-large` | 1/1 | **4/4 exact** (colour suit, badge seats, merged grids) |
| CARDS `intobridge-2-hand-large` | 1/1 | **E/W exact**; N/S hidden → `-` |
| CARDS `intobridge-4-hand-cramped` | 1/1 | legal read, **unpinned** |
| CARDS `intobridge-2-hand-small`, `-4-hand-small` | 0/1 | cross-scale mis-segment (soft-fail) |

Every non-valid board **soft-fails** (flagged partial), never a traceback
(session-2 containment). The `atlas/{realbridge,print,print-zoomed,bbo,
intobridge,realbridge-replay,...}` dirs + sessions 4-11 code diffs are
uncommitted.
...
5. **Print grids — tiling DONE, recognition is resolution-bound.** `print-3x4`
   tiles 12/12 via `detect._frame_tiles` (ruled board rectangles; no compass
   needed) + `print`/`print-zoomed` atlases ship. Recognition now tracks
   resolution: high-res "zoomed" exports read exact (`print-4x5-zoomed-format-high`
   **8/8**), but regular ~12px grids only partly (`print-3x4` board 1 exact, 3/12
   overall) — `_box_rows` over/under-segments the tiny compact glyphs. The proven
   answer is more pixels, not more knob-tuning (session 11); the low-res tail is
   diminishing-returns, left deliberately.
...
### 5. Only if a fully-clean BridgeWebs grid becomes necessary

The residual grid failure (`3x3-multi` 6/9) is render-mismatch recognition
confusion: the atlas was built from a standalone render, grid tiles render
slightly differently, so the same shapes misread the same way. Real fix is known
and cheap in principle: hand-label ONE grid tile (~52 glyphs) → same-render
atlas. (Session 11 already applied a lighter version of this — a **multi-render**
atlas built from both a standalone and a grid board took `…-part2` to **9/9**.)
Skip while grids stay majority-valid.
